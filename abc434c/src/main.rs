...
fn solve() {
    input! {
        n: usize,
        h: u32,
        checkpoints: [(u32, u32, u32); n]
    }
    let mut current_time = 0;
    let mut current_lowest = h;
    let mut current_highest = h;
    for (t, l, u) in checkpoints {
        let dt = t - current_time;
        if u + dt < current_lowest {
            println!("No");
            return;
        }
        if l > current_highest + dt {
            println!("No");
            return;
        }
        current_time = t;
        current_lowest = l.max(current_lowest.saturating_sub(dt));
        current_highest = u.min(current_highest + dt);
    }
    println!("Yes");
}
